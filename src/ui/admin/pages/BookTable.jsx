...
const BookTable = () => {
    ...
    const fetchBooks = async () => {
        try {
            ...
            // Filter out books with status "DELETED"
            const filteredBooks = res.data.filter(book => book.status !== 'DELETED');
            setBooks(filteredBooks);

            // Fetch images for non-deleted books
    ...
    const handleAddOrUpdateBook = async (bookData) => {
        try {
            ...
            fetchImage(res.data.imageName, res.data.id);
            setShowForm(false);
        } catch (error) {
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await axios.get(`http://localhost:8080/api/book/admin/delete/${id}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
        ...
        <div className="bg-[#262626] text-white p-4 rounded-xl mx-4 md:mx-10 overflow-x-auto">
                    ...
                    <tbody>
                        {books.map((book, index) => (
                            <tr key={book.id} className="bg-[#1e1e1e] text-left rounded-lg">
                                ...
                                <td className="px-4 py-2 border-t border-[#ffffff8a] text-center">
                                    {book.numberOfBooks}
                                </td>
            ...
            {showForm && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
                    <div ref={modalRef} className="bg-white text-black p-6 rounded-lg w-full max-w-lg relative">
                        <button onClick={closeModal} className="absolute top-2 right-2 text-gray-600 hover:text-black text-2xl">
                            &times;
                        </button>
                        <AddBook onClose={closeModal} onSubmitBook={handleAddOrUpdateBook} editData={editBook} />
                    </div>
                </div>
            )}
        </div>
